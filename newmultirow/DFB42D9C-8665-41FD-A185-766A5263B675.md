TipNotifyを使用すると、検証エラーをバルーンチップで通知できます。

## デザイナによる設定

デザイナでセルの検証と共にTipNotifyを使用するには、次のように操作します。
...
5. 表示されたCellValidateActionコレクションエディタでTipNotifyを追加する。
6. [メンバ]リストでTipNotifyが選択されていることを確認する。画面右のプロパティグリッドでToolTipTitleプロパティにタイトル、ToolTipTextプロパティに説明をそれぞれ入力する。
...
tipNotify1.ToolTipTitle = "タイトル";
tipNotify1.ToolTipText = "エラー";
