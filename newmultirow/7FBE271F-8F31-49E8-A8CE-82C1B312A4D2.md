列ヘッダは、ColumnHeaderCell を配置するための領域です。開発者は列ヘッダに複数のセクションを追加し、実行時に切り替えることができます。

## 列ヘッダ セクションの追加

列ヘッダ セクションは次のいずれかの方法で追加できます。
* メニューの［テンプレート］－［追加］－［列ヘッダ］を選択する。
* コンテキスト メニューの［追加］－［列ヘッダ］を選択する。
...
列ヘッダ セクションを削除するには、次の操作を行います。
1. 列ヘッダ セクションのタイトルにマウスカーソルを合わせ、コンテキスト メニューを開く。
